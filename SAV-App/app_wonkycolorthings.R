...
ui <- fluidPage(
  ...
  tags$div(
    ...
    tags$h2(("MD DNR's SAV Coverage"), style = "margin: 0; color: #00006e;"),
...
server <- function(input, output, session) {
  ...
  if(input$segment_selection == "") {
    ...
    SAVgg <- ggplot(SAVforgraph, aes(x=Year, y=Coverage))+
      ...
      annotate("text", 
               x=5, 
               y=ifelse(unique(SAVforgraph$RestorationAcreageGoal) == 0, 
                               0.1,
